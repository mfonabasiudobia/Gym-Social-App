                     ...
                     <section class="border-b md:border-r">
                        <section class="p-3 space-y-1 flex md:flex-col items-center justify-between md:justify-center">
                           ...
                           <div class="flex-1 p-2 md:flex md:flex-col md:items-center space-y-1">
                              <a href="{{route('user.dashboard',['username' => $follower->following->unique_name])}}" class="font-medium text-sm md:text-base">{{ucfirst($follower->following->name)}}</a>
                              <p class="text-xs md:text-sm opacity-70">Instructor</p>
                              <hr class="w-1/3 hidden md:block" />
                              <div class="text-xs opacity-50">{{$follower->following->followers->count()}} followers</div>
                           </div>
                           <hr />
                           <div>
                             <x-buttons.follow1 userId="{{$follower->from_id}}"/>
                           </div>
                        </section>
                     </section>
                  @endforeach
