...
<td :class="editable_data.return ? 'format-5' : editable_data.showroom_formatted ? 'format-6' : ''">

        @if(in_array(session('current_tab'),['inventory', 'inventory-in', 'inventory-out']))
            <button        
                x-cloak
                :class="!is_row_editable ? 'hidden' : ''"
                class="px-1" 
                x-on:click="$wire.emit('updateInventory', { data : editable_data }); is_row_editable = false; "
            >
                <i class='las la-save'></i>
            </button>
        @endIf

        <button class="px-1" x-on:click="$dispatch('trigger-power-edit', {'id' : {{$row->id}} })">
            <i class='las la-pencil-alt'></i>
        </button>

        <button class="px-1" x-on:click="$dispatch('set-delete', ['{{session('model')}}', {{$row->id}}])">
